...
void HandGesture::drawFingerTips(MyImage *m){
	...
	for(int i=0;i<fingerTips.size();i++){
		...
		if(old_number!=mostFrequentFingerNumber && (mostFrequentFingerNumber==1||mostFrequentFingerNumber==2||mostFrequentFingerNumber==3||mostFrequentFingerNumber==4||mostFrequentFingerNumber==5||mostFrequentFingerNumber==0)&& button_speech)//here whether given code will run or not is determined by button_speech
			{
				
			//outfile <<mostFrequentFingerNumber<<endl;
			words=mostFrequentFingerNumber;
			old_number=mostFrequentFingerNumber;
			//outfile.close();
			//outfile.flush();
